...
### ✅ Этап 8.5 — TTS и системные звуки (дополнительный подэтап)
- [x] Добавлен пакет `flutter_tts` для озвучки китайского текста
- [x] Расширен `AudioService`: методы `playSfx()` (поддержка .wav/.mp3) и `speakChineseText()`
- [x] Создана папка `assets/audio/sfx/` с файлами correct.wav, wrong.wav, achievement.wav, click.wav, complete.wav
- [x] Озвучка китайского слова при клике на чип в `WordMatchingExercise`
- [x] Озвучка китайского слова при клике на целевую карточку в `ToneSelectionExercise`
- [x] Озвучка китайского слова при клике на карточку цели в `TranslationExercise` (также показ перевода)
- [x] Озвучка китайского слова при клике на карточку в `WordCardFlipExercise` (на обеих сторонах)
- [x] Озвучка китайского слова при клике на вариант ответа в `FillInTheBlankExercise`
- [x] Озвучка китайского слова при клике на токен-слово в `SentenceBuilderExercise`
- [x] Интеграция TTS в `DialogueExercise` (клик по реплике озвучивает китайский текст)
- [x] Интеграция TTS в `BlockTextScreen` (клик по слову в тексте)
- [x] Интеграция TTS в `TopicScreen` (клик на слово-чип)
- [x] Звуковая обратная связь correct/wrong во всех упражнениях: `WordMatchingExercise`, `ToneSelectionExercise`, `TranslationExercise`, `TrueOrFalseExercise`, `FillInTheBlankExercise`, `SentenceBuilderExercise`, `WordCardFlipExercise`, `DialogueExercise`, `ListeningExercise`
...
- [x] `AudioService` — поддержка системных звуков (SFX) через `assets/audio/sfx/` (correct.wav, wrong.wav)
- [x] `AudioService` — интеграция TTS для озвучки китайского текста через `flutter_tts` с авто-прерыванием
- [x] Добавлен `just_audio_windows` для поддержки Windows
...
- [x] Озвучка китайского текста в упражнениях: иконка 🔊 или клик по слову в `WordMatchingExercise`, `ToneSelectionExercise`, `TranslationExercise`, `WordCardFlipExercise`, `FillInTheBlankExercise`, `SentenceBuilderExercise`, `DialogueExercise`, `BlockTextScreen`
...
- [ ] Настройка скорости TTS в SettingsScreen
...
├── core/
│   ├── audio/
│   │   └── audio_service.dart         ← Singleton аудио-плеер + TTS + SFX. Provider.
│   ├── services/
...
│   └── sfx/                           ← correct.mp3, incorrect.mp3 — системные звуки.
└── fonts/
...
|------|--------------|---------------|
| `lib/core/audio/audio_service.dart` | Аудио-воспроизведение, системные звуки, TTS | Единственный источник аудио. Ошибка = нет звука во всём приложении |
...
| `lib/core/widgets/app_card.dart` | Все карточки везде | `AppCard`, `AppBackground`, `AppChip` используются в 15+ файлов. `AppBackground` нельзя дублировать в других файлах — будет конфликт импортов |
